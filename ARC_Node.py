...
@File: ARC_Node.py
@Author: Wang Yang
@Date:   2024/12/15 19:05 
@Last Modified by: Wang Yang
@Descirption : ARC 结合 Node2vec
'''
...
class ARC_Node(nn.Module):
    def __init__(self, data, in_feats, h_feats=32, num_layers=2, dropout_rate=0, activation='ReLU', num_hops=4, **kwargs):
        ...
        self.node2vec = Node2Vec4AD(data.edge_index,
                                    embedding_dim=64,
                                    walk_length=20,
                                    context_size=10,
                                    walks_per_node=10,
